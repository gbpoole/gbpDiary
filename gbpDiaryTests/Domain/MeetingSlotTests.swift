...
struct MeetingSlotTests {
    ...
    @Test func slots_exactlyAtNoon_isAfternoon() {
        let m = minutes(startHour: 12)
        let result = MeetingSlotClassifier.slots(for: m, on: referenceDay, calendar: calendar)
        #expect(result == [.afternoon])
    }

    @Test func slots_endsExactlyAtNoon_spansNoon() {
        // 11:59 + 1 min = 12:00 → end >= noon
        let m = minutes(startHour: 11, startMinute: 59, durationHours: 1.0 / 60.0)
        let result = MeetingSlotClassifier.slots(for: m, on: referenceDay, calendar: calendar)
        #expect(result == [.morning, .afternoon])
    }
}
