...
`WorkspaceTab` is `.diary | .chat | .triage | .tasks | .projects | .people | .institutions | .meetings | .documents | .content | .images | .tags | .timesheet` plus entity cases `.project/.person/.institution/.minutes/.document/.contentNote/.task/.curation(PersistentIdentifier)` (`.curation` carries the **root project** of a curation walk and sits under the Projects category). Category tabs render the existing list/tool views (`.chat` → `ChatView`, `.triage` → `EmailTriageView`, `.content` → `ContentListView`); the **Emails** sidebar item (`.triage`) carries a `.badge` of the store-wide un-triaged (`unclassified`) email count; entity tabs resolve the model via `modelContext.model(for:)` and render its `…DetailView` (in non-sheet mode; `.contentNote` → `ContentNoteDetailView`, `.task` → `TaskDetailView`). `reveal(note:)` sends a content note to its own `.contentNote` tab (checked before the project fallback). **`revealTimeEntry(_:)`** / **`revealFocusBlock(_:)`** focus the diary on a logged `TaskTimeEntry`'s (resp. a `FocusBlock`'s) day (Day mode) and set `DiaryState.scrollTargetEntryId` / `scrollTargetBlockId` so the Activity section scrolls to it (each `ActivityEntryRow` carries `.id(entry.id)`, each `FocusBlockRow` `.id(block.id)`) — used by the task detail page's Time Log.
...
- **Rich entities** (`Project`, `Person`, `Document`, `Task`) → open the entity **in a workspace tab**
  (`workspace.focusOrOpen(.project(id))` / `.person(id)` / `.document(id)` / `.task(id)`) — the tab's detail
  view browses related items and edits metadata inline (no pencil). `Minutes` rows open `MinutesDetailView`
  (edit-in-place), which is already one-click. New people are created via the People page "+" button
  (`PersonEditorSheet`), not by opening a row. **`TasksView` double-click** opens `TaskDetailView` in a tab
  (`.task(id)`); its metadata editing stays in `TaskEditorSheet` (reached via the detail page's Edit action
  + the table's context/bulk menu) because a task has far more fields than Project/Person.
...
per-tab `TasksFilterState.viewMode` (`TaskViewMode`: **Reviewed / Triage**): *Reviewed* is
the existing filterable table scoped to `!(isOpen && needsTriage)` (closed tasks always show — no need to
triage a done task); *Triage* is a **multi-selectable** `List` of inbox tasks — a **"Due & overdue"** section first (the only place overdue captures surface, since they can't reach the board), then the oldest-first backlog — with its own `BulkActionBar` — **Mark reviewed / Add to board / Cancel / Delete** — where mark-reviewed honours the same rule as the row button (a task needs a project first) and **reports how many it skipped**; rows are **`TaskTriageRow`**
(`Views/DayTab/TaskTriageRow.swift` — status + summary, then grouped inline **project / due / scheduled /
priority** setters and a **Reviewed** button; parallels `EmailTriageRow`). The **Tasks sidebar
...
- `TaskEditorSheet` — full task editing sheet. A **Parent task** `FuzzyPickerField` makes the task a subtask of another; candidates exclude the task's **own subtree** via `TaskParenting` (parenting to a descendant would fold the tree into a ring and make every walk over it — breakdown, board hierarchy, time roll-up — loop or lose tasks). Gaining a parent clears `dayRecord`/`originMinutes`, since container ownership belongs to root tasks only (the same rule `DiaryTaskRow` applies on drag). Carries a **Subtasks** outline field (below Notes) whose text is materialised via `TaskBreakdown.create` **on save** — for a new task after the insert, so the subtree attaches to a task the context already knows; for an existing task it appends.  Accepts `task: Task?` (nil = create new) and `defaultDate: Date`. New tasks default to unscheduled; notes field has a visible rounded border. When editing an existing task, a "Time Log" section shows all `TaskTimeEntry` items with an "Add Entry…" button opening `LogTimeSheet`. Optional save gates `requireAssignee` (meeting action items) and `requireProject` (focus-block task creation) flow through the pure `TaskEditorValidation.canSave`; `onTaskCreated` reports a newly-created task to the caller (e.g. the focus-block picker auto-selects it). New tasks default their assignee to the "Me" person (`AppSettingsStore.myPersonID`).
...
- `FocusBlockEditorSheet` — sheet for creating or editing a `FocusBlock`. **Time is only ever assigned to tasks**, so a standard block is always task-backed (a single task picker — no Task/Project source toggle); an evening block is a pure overtime container with no task. `save()` always clears the legacy `block.project`. The task picker (`FocusBlockTaskPickerRow`) stacks a **single-select, searchable "Project" `FuzzyPickerField`** (all non-completed projects; empty = "All projects") above the **"Task"** `FuzzyPickerField` — fuzzy search scales to any number of projects (a flat chip row did not). The selected project narrows the active (todo/started) task list via the pure `FocusBlockTaskFiltering.filter` (passed a set of 0 or 1 id). "Create new" routes through a **seeded `TaskEditorSheet`** (`presetSummary` = typed text, `presetProject` = the selected project, `requireProject: true`, assignee defaulting to "Me"); its `onTaskCreated` auto-selects the new task back into the block.
...
- `StandingTasksSection(project:)` (Views/Shared) — a project's **standing tasks**: perpetual, stateless work that never completes ("keep the inbox at zero"). Rendered on both `ProjectDetailView` and `CurationView`, with an inline add field that creates the task and calls `makeStanding()` (which also clears `needsTriage` and any horizon, so a standing task can never block a curation gate or reach the board); the new task's assignee defaults to the "Me" person. `StandingTaskRow` shows **no status control** — a standing task has no status cycle — just its logged total and a log-time button; double-click opens the task tab. A project's **Open Tasks** list excludes standing tasks (they would otherwise sit there forever).
- `StandingTaskStrip(date:)` (Views/DayTab) — the diary's **fixed standing strip**, pinned below the scrolling buckets in `DayTaskPanel` (scoped to "Me" like the panel). Collapsible, showing each standing task with its project, the hours logged **on the shown day**, and one-click **15m / 30m / 1h** quick-log buttons. Time books via `WeekendPolicy.logNowDate` (the same rule as the panel's quick-add, so weekend work lands in Friday's overtime) and **does not** flip the task to `.started` — there is no status to advance.
- `BoardPanel` — the **planning board**, hosted as an `.inspector()` panel beside the Tasks table (not a tab: it was one, and the `"board"` session token now **migrates to `.tasks`** so old sessions keep their tab). Three lanes — **Today / This Week / Maybe**; there is no Inbox lane, because tasks arrive from the table next door where untriaged work is revealed by its own filter, so placing a task IS the triage act. **Placement is manual and never expires** — nothing auto-demotes at a day or week boundary. Lanes render **nested** (`BoardHierarchy.rows`): a parent row with its subtasks inside. **Ancestors are context, never members** — an ancestor pulled in to place a subtask carries no horizon, is dimmed, has no status control, and shows only that lane's children, never siblings; it disappears when its last member goes. **Placing or moving a row takes its whole open subtree** (`BoardHierarchy.placementTargets`), so a breakdown never splits across lanes — which is what keeps an ancestor row unambiguous; a task with no open descendants places just itself. Top-level groups order by `planSortOrder`, rows within a group by the tree's own `Task.sortOrder`. Tasks arrive from the table next door — **multi-select → "Add to board ▸ …"** in `BulkActionBar` or the row context menu, or **drag rows straight into a lane** (a `TableRow` drag carries the whole selection, non-contiguous included — spiked before building on it). Closed tasks can't be placed, so a bulk add **reports how many it skipped** instead of silently doing nothing. Cards are **selectable** — click to select, ⌘/⇧ to extend, click empty lane space to clear; selection spans lanes. Drag a card between lanes to move it, or **drag it onto the Tasks pane on the left to unplan it** (the inverse gesture: right to plan, left to unplan — verified by spike that a drop registers there despite the pane containing an `NSTableView` whose own rows are drag sources). Dragging a **selected** card takes the whole selection; since SwiftUI's `.draggable` carries only one item, the selection travels as a single `BoardDragPayload` string that drops decode. Removal always takes the row **and its descendants**, leaving siblings and ancestors alone. The card context menu (Move to… / **Remove from board**) and the lane header's **Clear lane** remain as the precision path. `BoardPlacement` holds the two rules: a placed task **appends to the end** of its lane (`appendOrder`, max+1 not count) and **only open, triaged tasks may be placed at all** (`canPlace`). **RULE: triage is a gate, not something planning does for you** — placing no longer reviews anything (it used to; that was deliberately reversed), the board never contains or shows inbox work, and a bulk add skips untriaged and closed tasks, **naming which rule skipped what**. The Today lane additionally shows a **derived "Due & overdue" section above its manual cards** (`BoardDueGroup`): open, triaged, non-waiting, non-standing, **unplaced** tasks due today or earlier, most overdue first. **Nothing is written** — membership is recomputed every render, so `planHorizon` stays purely manual and a task leaves by having its date changed, being planned, or being completed. Those cards are **read-only** (no drag, a left stripe in the due colour, status control kept so you can complete in place); their only action is **"Plan it"**, which graduates the task into the manual section below. **Standing tasks are placeable** — perpetual work like "Triage Emails" is legitimately planned into a day, and since it never completes it leaves a lane only by being removed. Geometry is measured, not guessed: `inspectorColumnWidth(min: 560, ideal: 780, max: 900)` with the content pane at `.frame(minWidth: 360)` (without it the table collapses to ~100pt; a 700 minimum imposed a 1461pt floor on the whole window). **RULE: the auto-hide rule keys off the WINDOW width** (`WindowWidthReader`), never the content column — hiding the panel widens the content, which would re-satisfy the show condition and oscillate — with hysteresis (hide < 1150, show > 1250) and the write deferred out of the update pass. The toggle sits **beside the Reviewed/Triage picker** — it is a separate on/off, not a third view mode, since the board shows alongside whichever list you are in. Visibility is **per Tasks tab** (`WorkspaceTabState.boardPanelShown`), persisted as an **optional** snapshot field so sessions written before the panel existed still decode.
- `CurationView(root:)` — the `.curation` tab: a **linear stepper** for working through a neglected project's tasks at scale. Launched from the Projects table's **"Curate tasks…"** context menu (`workspace.openInNewTab(.curation(id))`). The order is the pure `CurationWalk.order` — root first, then **active** subprojects depth-first (completed subtrees pruned), siblings by name — recomputed from live data each render. Page-styled like the other detail views: pinned header (project name, `N of M` progress, an **"N to review"** / "All reviewed" chip from `ProjectCurationRules.unreviewedCount`, and the last-reviewed stamp) + a `DayActionBar` with **Previous / Mark reviewed / Skip / Open project**. **Mark reviewed is gated by `ProjectCurationRules.canMarkTasksReviewed`** — disabled while any open task still needs triage — and on success stamps `Project.tasksReviewedAt` and advances (the last project returns to the Projects list). Body: a **To review** section of `TaskTriageRow`s (the project's untriaged open tasks) then **Open tasks** as a `TaskSubtreeView` + a `TaskBreakdownField`. Walk position lives on `WorkspaceTabState.curationIndex` (session-only) so switching tabs doesn't restart the session; breakdown drafts are keyed per project.
- `TaskDetailView(task:asSheet:)` — the `.task` tab (opened by double-clicking a `TasksView` row, like Project/Person), page-styled like `ProjectDetailView` (`AppTheme.background`, pinned header + `DayActionBar`, scrolling `DaySectionHeader` sections). Header: summary title then a **minutes-style inline-editable metadata card** (`AppTheme.cardRaised.opacity(0.3)`, `metaRow`-based — the same shape as `ProjectDetailView` / `PersonDetailView`): Summary / Status (`TaskStatusMenu`) / Project / Assignee (`FuzzyPickerField`, create-inline) / Priority (segmented) / Due / Scheduled (toggle + `DatePicker`, clearable) / Tags / Notes / **Blocked by** (`FuzzyPickerField` over cycle-safe `blockerCandidates`) / **Repeats** (`RecurrenceRule.parse`) / **Wait until** / **Until**, each binding straight to the model and stamping `updatedAt`. **RULE: the task page edits every task field in place — it opens no `TaskEditorSheet` at all** (no pencil, no "more fields" escape hatch); a subtask's `onEdit` opens that subtask's own `.task` tab per the row-tap rule. Read-only context (the source chip, **Blocking** — the inverse dependency side) is displayed but not editable; the **logged total is reported as a "Total" footer row in the Time Log section**, beside the entries that make it up, not as a header chip. The sole `DayActionBar` action is **Add time entry** (→ `LogTimeSheet(presetTask:)`). The primary section is **Time Log** — a single newest-first list merging the task's **`TaskTimeEntry`s and the `FocusBlock`s it backs** (a block row shows its **net** hours via `TimeLedgerProjection.blockNet`), each row clickable (`workspace.revealTimeEntry` / `revealFocusBlock`) to jump to the diary where it lives — plus a **Subtasks** section that is **always shown** (so a task with none can still be broken down): `TaskSubtreeView` for the row controls over `TaskBreakdownField` for batch outline entry. That section's **Total** row uses `TaskTimeReport.totalHours` (logged entries + focus-block net).
...
| Meeting counted by in-slot portion (no double-count across morning/afternoon): `MeetingSlotHours.overlapHours(start:durationHours:slot:on:)` = the meeting's hours within a slot's range (morning `[startOfDay,12:30)`, afternoon `[12:30,endOfDay)`, allDay/evening = full). `FocusBlockRow.netHours` + `ActivitySection.meetings(for:)` (overlap>0 filter) + `MeetingActivityRow` use it, and `TimeLedgerProjection` splits a weekday meeting across the day's blocks by overlap (each net reduced by its part; total = one meeting) — so an 11:30–12:30 meeting is 1h morning / 0 afternoon, a 12:00–13:00 meeting is 0.5/0.5 | Activity section / focus-block meeting time | `gbpDiaryTests/Domain/MeetingSlotHoursTests.swift`, `gbpDiaryTests/Domain/TimeLedgerProjectionTests.swift` | `meetingEndingExactlyAt1230_isAllMorningNoAfternoon`, `meetingStraddling1230_splitsInHalf`, `whollyAfternoonMeeting`, `allDayIsNeverSplit`, `zeroDuration_isZero`, `spanningMeeting_splitsHalfIntoMorningAndAfternoonBlocks`, `meetingEndingAt1230_countsFullyInMorning` |
...
| Corrective focus-block description migration: `FocusBlockDescriptionMigration.plan` groups blocks with a non-blank (trimmed) `comment` by (projectID, description) into one `Group` each (blank/nil description omitted; input order preserved), so the flag-guarded `WorkspaceView.migrateFocusBlockDescriptionsOnce` backs each group with one completed task named by the description and re-points its blocks (Obsidian-imported blocks had project-named tasks + the description in the comment) | Focus blocks / import correction | `gbpDiaryTests/Domain/FocusBlockDescriptionMigrationTests.swift` | `blankOrNilDescription_omitted`, `singleDescribedBlock_oneGroup`, `sameProjectAndDescription_sharesOneTask`, `descriptionTrimmedBeforeGrouping`, `differentProjectOrDescription_splits`, `preservesInputOrder` |
| Add-focus-block task filter: `FocusBlockTaskFiltering.filter` keeps only active (open) tasks, then — when the project-chip selection is non-empty — those whose project id is in the set (OR across projects; a project-less task is dropped); an empty selection means all projects | Focus blocks / task picker | `gbpDiaryTests/Domain/FocusBlockTaskFilteringTests.swift` | `emptySelection_returnsAllActiveTasks`, `inactiveTasks_alwaysExcluded`, `singleProject_keepsOnlyThatProject`, `multipleProjects_areOred`, `projectlessTask_excludedWhenFilterActive` |
| Task editor save gate: `TaskEditorValidation.canSave` requires a non-blank summary, and (when the respective flag is set) a project (`requireProject` — focus-block task creation) and/or an assignee (`requireAssignee` — meeting action items) | Task creation / focus blocks | `gbpDiaryTests/Domain/TaskEditorValidationTests.swift` | `blankSummary_cannotSave`, `nonBlankSummary_noRequirements_canSave`, `requireProject_blocksUntilProjectChosen`, `requireAssignee_blocksUntilAssigneeChosen`, `bothRequirements_needBoth` |
...
| `Task.loggedHoursNormalized` sums `hoursNormalized` across all `timeEntries`; returns 0 when empty. `Task.loggedDuration` returns nil when no entries, else a Duration in hours | Timesheet / Activity section | `gbpDiaryTests/Models/TaskComputedPropertyTests.swift`, `gbpDiaryTests/Domain/TaskTableVisibilityTests.swift` | `loggedHoursNormalized_sumsAllTimeEntries`, `loggedHoursNormalized_emptyEntries_returnsZero`, `loggedDuration_returnsNilWhenNoEntries`, `loggedDuration_returnsNonNilWithSummedHours` |
| `TaskTimeReport.totalHours` (Tasks-table "Time" column + task Time Log total) = logged time entries + the NET of the task's focus blocks (from the canonical `TimeLedgerProjection.blockNet`); the legacy `duration` is used only when there are no entries AND no blocks; 0 when nothing is logged; entries+blocks win over the legacy fallback | Tasks page / time column + task detail | `gbpDiaryTests/Domain/TaskTimeReportTests.swift` | `entriesOnly_sumsEntries`, `blocksOnly_sumsNet`, `entriesPlusBlocks_sumsBoth`, `legacyFallback_onlyWhenNoEntriesAndNoBlocks`, `zeroWhenNothingAtAll`, `entriesWinOverLegacy` |
...
| Task detail tab: `.task(id)` is an entity tab (category `.tasks`) opened by double-clicking a `TasksView` row; `focusOrOpen(.task)` opens then reuses it, `closeEntity`/`references` cover it, and it snapshots/restores via the `"task"` `entityKind` token. `revealTimeEntry(_:)` / `revealFocusBlock(_:)` focus the diary on the entry's / block's (weekend-folded) day in Day mode and set `DiaryState.scrollTargetEntryId` / `scrollTargetBlockId` | Navigation / task tab + time reveal | `gbpDiaryTests/Views/WorkspaceModelTests.swift`, `gbpDiaryTests/Domain/WorkspaceSessionTests.swift` | `task_tab_focusOrOpenReusesAndCloseEntityCloses`, `snapshot_capturesTaskEntity_andRestoresIt`, `revealTimeEntry_focusesDiaryOnEntryDayWithScrollTarget`, `revealFocusBlock_focusesDiaryOnBlockDayWithScrollTarget`, `entityKind_tokens` |
...
| Planning horizon value type: `PlanHorizon` (today/thisWeek/maybe) `rank` orders Today→This Week→Maybe; `displayName` human-readable; rawValue round-trips | Task management / planning board | `gbpDiaryTests/Domain/PlanHorizonTests.swift` | `rank_ordersTodayThisWeekMaybe`, `displayName_isHumanReadable`, `rawValue_roundTrips` |
| Standing tasks + planning placement (Task model): a new task is not standing; `makeStanding()` sets the flag, clears `needsTriage`, and clears any horizon; `planHorizon` round-trips through `planHorizonRaw` (nil clears); `place(on:)` sets the horizon but is a no-op for standing tasks; `isOverdue` is exempt for standing tasks | Task management / standing tasks | `gbpDiaryTests/Models/TaskComputedPropertyTests.swift` | `newTask_isNotStandingByDefault`, `makeStanding_setsFlagClearsTriageAndHorizon`, `planHorizon_roundTripsThroughRaw`, `place_setsHorizon_butStandingTasksIgnoreIt`, `isOverdue_exemptForStandingTasks` |
| Standing-task urgency exemption: `TaskUrgency.score` returns 0 when `isStanding` (like `!isOpen`), so standing tasks are never ranked in actionable lists | Task management / urgency | `gbpDiaryTests/Domain/TaskUrgencyTests.swift` | `standing_scoresZero` |
| Curation reviewed-gate: `ProjectCurationRules.canMarkTasksReviewed(openTaskNeedsTriage:)` is true only when no open task still needs triage (empty = ok, mirrors `ProjectStatusRules`); `unreviewedCount` counts the untriaged | Task management / curation stepper | `gbpDiaryTests/Domain/ProjectCurationRulesTests.swift` | `canMarkTasksReviewed_requiresNoUntriagedOpenTask`, `unreviewedCount_countsUntriaged` |
| Project last-activity: `ProjectActivity.lastActivityAt(timeEntryDates:focusBlockDates:)` = the max of all supplied dates (nil when empty); `Project.lastActivityAt` gathers its OWN tasks' time-entry + focus-block dates (not subprojects) → that max or `.distantPast`; `Project.tasksReviewedKey` = `tasksReviewedAt` or `.distantPast` | Task management / Projects columns | `gbpDiaryTests/Domain/ProjectActivityTests.swift`, `gbpDiaryTests/Models/ProjectSortKeysTests.swift` | `lastActivityAt_nilWhenNoWork`, `lastActivityAt_isMaxAcrossBothSources`, `lastActivityAt_usesTimeEntriesWhenNoBlocks`, `lastActivityAt_isLatestWorkOnOwnTasksOrDistantPast`, `tasksReviewedKey_isStampOrDistantPast` |
| Curation walk: `CurationWalk.order(rootID:…)` emits the root then its ACTIVE subprojects depth-first (completed subprojects + their subtrees pruned; root always included even if completed), siblings ordered by the comparator, cycle-safe; unknown root → empty | Task management / curation stepper | `gbpDiaryTests/Domain/CurationWalkTests.swift` | `rootThenActiveSubprojects_depthFirst`, `completedSubprojectAndSubtreePruned`, `rootIncludedEvenIfCompleted`, `siblingsOrderedByComparator`, `unknownRoot_isEmpty`, `cycleTerminates` |
| Planning-board partition: `BoardPartition.partition` excludes standing + closed tasks; a `needsTriage` task → Inbox (over any stale horizon); else routes by `planHorizon` (nil = off-board backlog, in no section); each section stable-sorts by `planSortOrder` | Task management / planning board | `gbpDiaryTests/Domain/BoardPartitionTests.swift` | `inboxIsNeedsTriage_regardlessOfHorizon`, `horizonsRouteToTheirSections`, `reviewedWithNoHorizon_isOffBoard`, `standingAndClosed_areExcluded`, `sectionsSortByPlanSortOrder_stableOnTies`, `empty_returnsEmptyBuckets` |
| Task-outline parse: `TaskOutlineParser.parse` turns indented/prefixed text into an `OutlineNode` tree — leading whitespace = depth (tabs = one level, spaces ÷ indentWidth=4), a `-`/`*`/`+` marker stripped, blank lines ignored, an over-indent jump clamped to parent+1, a first indented line still a root, a bare marker with no text dropped | Task management / breakdown quick-add | `gbpDiaryTests/Domain/TaskOutlineParserTests.swift` | `flatLines_becomeRoots`, `indentationNestsChildren`, `tabsCountAsOneLevel`, `listMarkersAreStripped`, `blankLinesIgnored`, `overIndentJumpClampsToParentPlusOne`, `firstLineIndented_isStillARoot`, `emptyText_isEmpty`, `outdentReturnsToAncestor`, `bareHyphenWithNoText_isIgnored` |
| Task time roll-up: `TaskTimeRollup.subtreeHours(rootID:ownHours:childrenByParent:)` = a node's own hours + all descendants' own hours (reporting aggregation only, never changes attribution); leaf = own hours; a child's subtree excludes siblings; cycle-safe (each node counted once) | Task management / time roll-up | `gbpDiaryTests/Domain/TaskTimeRollupTests.swift` | `leaf_isJustOwnHours`, `leaf_withNoLoggedHours_isZero`, `parent_sumsOwnPlusDescendants`, `subtreeOfChild_excludesSiblings`, `cycle_countsEachNodeOnce` |
...
| First-class task provenance: `@Model TaskSource` (kind + url + title + optional `email`) is `Task.source` (cascade: deleting the task deletes its source); deleting the linked `EmailMessage` nullifies `TaskSource.email` but the source survives. `TaskSourceMigration.plan` selects email tasks (legacy `originEmail`, no `source`) for the flag-guarded launch back-fill (`WorkspaceView.migrateTaskSourcesOnce`); email tasks keep `originEmail` as the inverse backbone and carry both | Task provenance / capture | `gbpDiaryTests/Models/RelationshipIntegrityTests.swift`, `gbpDiaryTests/Domain/TaskSourceMigrationTests.swift` | `taskDelete_cascadesSource`, `emailDelete_nullifiesTaskSourceEmail_sourceSurvives`, `plansEmailTasksWithoutASource`, `skipsTasksThatAlreadyHaveASource`, `skipsTasksWithNoOriginEmail`, `emptyInput_isEmpty` |
| Quick capture: `CaptureURL.parse` reads `gbpdiary://task?title=&kind=email\|slack\|web\|other&url=&mailId=` (percent-decoded, kind whitelist→`.other`, rejects wrong scheme / non-`task` action / empty content). `gbpDiaryApp.onOpenURL` presents `QuickCaptureView`, which creates a `Task` (**always `needsTriage`** → Inbox) + a `TaskSource` (email `mailId` resolved to an `EmailMessage` by `messageId`), warns on a duplicate open `source.url`. The task row glyph + `TaskDetailView` Source chip read `task.source` (chip opens the link/Mail) | Quick capture / URL scheme | `gbpDiaryTests/Domain/CaptureURLTests.swift` | `parsesWebCapture`, `parsesEmailCaptureWithMailId`, `kindDefaultsToOther`, `unknownKindFallsBackToOther`, `rejectsWrongScheme`, `rejectsNonTaskAction`, `rejectsEmptyContent` |
...
| Hierarchical Tasks table: `TasksView` lays rows out as a parent → child task tree through the generic, pure `ProjectHierarchy.rows` (shared with Projects, not reimplemented) over `TaskRow` items — a matching subtask keeps its whole ancestor chain as dimmed context (`isMatch == false`), a matching parent does not reveal non-matching subtasks, and siblings honour the table's sort comparator. The hierarchy (and its one `TimeLedgerProjection.blockNet` pass) is computed **once per render** in `taskTable` and captured by the cell closures — never via a computed property read per cell | Tasks page / hierarchy | `gbpDiaryTests/Domain/TaskHierarchyTests.swift` | `testMatchingSubtask_keepsAncestorsAsDimmedContext`, `testMatchingParent_doesNotRevealNonMatchingSubtasks`, `testSiblings_orderedByComparatorUnderTheirParent`, `testDeepSubtaskMatch_keepsWholeAncestorChain`, `testNoMatches_isEmpty` |
| Task breakdown (hybrid): `TaskBreakdown.create` materialises a parsed `[OutlineNode]` (`TaskOutlineParser`) into a task subtree — outline order preserved as `sortOrder`, siblings **appended** after the parent's existing children (never reshuffled), project/assignee inherited from the parent unless overridden, and `needsTriage` **inherited from the parent** so decomposing a reviewed task yields reviewed subtasks (which the Reviewed table would otherwise hide) while a parentless breakdown stays fresh capture in the Inbox; empty/whitespace text creates nothing. `TaskBreakdownField` (Views/Shared, reused by the Phase C stepper) is the batch input; `TaskSubtreeView` supplies the row controls (drag-reorder, drag-onto-row to indent, Detach from Parent to outdent) in `TaskDetailView`'s Subtasks section | Task management / breakdown workflow | `gbpDiaryTests/Domain/TaskBreakdownTests.swift` | `createsNestedSubtreeInOutlineOrder`, `inheritsProjectAndAssigneeFromParent`, `explicitProjectOverridesInheritance`, `subtasksInheritParentTriageState`, `rootLevelBreakdownStaysUntriaged`, `appendsAfterExistingChildrenWithoutReshuffling`, `emptyOrWhitespaceTextCreatesNothing` |

| Subtask-breakdown drafts survive tab switches: the unsaved outline lives in `WorkspaceTabState.breakdownDrafts` (keyed by task UUID) rather than view `@State`, because a tab's content is torn down and rebuilt on every switch; drafts are per task, and are **session-only** (never written to `WorkspaceSnapshot`, like `chatState`). `TaskBreakdownField` takes the storage as a `@Binding`, keeps the text on Cancel (only clearing it once tasks are actually created), and flags a kept draft in its collapsed label | Task management / breakdown workflow | `gbpDiaryTests/Views/WorkspaceModelTests.swift` | `breakdownDraft_survivesSwitchingTabsAndComingBack`, `breakdownDrafts_areKeyedPerTask`, `breakdownDraft_isNotPersistedInTheSnapshot` |

| Curation stepper: the `.curation(rootProject)` tab walks `CurationWalk.order` (root, then active subprojects depth-first) with per-tab position in `WorkspaceTabState.curationIndex` (session-only); it is an entity tab keyed by the root project — `focusOrOpen` reuses it, `closeEntity`/`references` cover it, and it snapshots/restores via the `"curation"` `entityKind` token. **Mark-reviewed is gated by `ProjectCurationRules.canMarkTasksReviewed`** and stamps `Project.tasksReviewedAt`; the Projects table exposes `lastActivityAt` / `tasksReviewedKey` as sortable neglect columns and launches a session from its context menu | Task management / curation stepper | `gbpDiaryTests/Views/WorkspaceModelTests.swift`, `gbpDiaryTests/Domain/WorkspaceSessionTests.swift` | `curationTab_focusOrOpenReusesAndCloseEntityCloses`, `snapshot_capturesCurationEntity_andRestoresIt`, `curationIndex_isHeldPerTab`, `entityKind_tokens` |

| Planning board: `BoardPartition` has **three** lanes (no Inbox bucket) and excludes only **closed** tasks — tasks needing triage and **standing** tasks are both placed like anything else; `Task.place(on:)` no longer no-ops for standing tasks and `makeStanding()` keeps any horizon; `BoardPlacement.appendOrder` puts a placed task at the **end** of its lane (max+1) and `shouldReview` marks an untriaged one reviewed | Task management / planning board | `gbpDiaryTests/Domain/BoardPartitionTests.swift`, `gbpDiaryTests/Domain/BoardPlacementTests.swift`, `gbpDiaryTests/Models/TaskComputedPropertyTests.swift` | `needsTriageTask_landsInItsHorizonLane`, `closedExcluded_standingIncluded`, `horizonsRouteToTheirSections`, `reviewedWithNoHorizon_isOffBoard`, `standingTaskCanBePlaced`, `appendOrder_goesAfterEverythingInTheColumn`, `place_setsHorizon_includingForStandingTasks`, `makeStanding_setsFlagClearsTriageAndDueDate_keepingAnyHorizon` |
| Board-tab retirement: the board is a panel on Tasks, so `WorkspaceCategory.board` and `WorkspaceTab.board` are gone and `WorkspaceTabCoding.categoryTab(forToken:)` **maps the legacy `"board"` token to `.tasks`** rather than returning nil (which would silently drop that tab at restore); `TabSnapshot.boardPanelShown` is **optional** so pre-existing sessions still decode | Session persistence / board migration | `gbpDiaryTests/Domain/WorkspaceSessionTests.swift` | `legacyBoardToken_migratesToTasks`, `categoryTokens_roundTrip`, `snapshot_jsonRoundTrips` |

| Standing tasks are perpetual and live outside the normal task flow: `DayTaskBuckets.partition` **excludes them from every bucket** (including the To Do catch-all) because they are logged from the diary's `StandingTaskStrip` instead; `BoardPartition` already excludes them and `Task.place` is a no-op for them; `ProjectDetailView`'s Open Tasks list excludes them in favour of `StandingTasksSection`; `TaskUrgency.score` returns 0 and `isOverdue` is exempt. The Tasks table **hides them from the Reviewed list unless the Standing flag filter is on** (like Waiting) and marks them with an `infinity` glyph. All three hide rules — untriaged / waiting / standing — live in the pure `TaskTableVisibility.isHidden`, and each yields **only to its own** reveal filter: `flag.needsTriage` shows the inbox inside the Reviewed table **for filing** — it is *not* a route to the board, since untriaged work cannot be planned — without dragging waiting or standing work in with it | Task management / standing tasks | `gbpDiaryTests/Domain/DayTaskBucketsTests.swift`, `gbpDiaryTests/Domain/BoardPlacementTests.swift`, `gbpDiaryTests/Models/TaskComputedPropertyTests.swift` | `standingTasks_excludedFromEveryBucket`, `standingTaskStaysOffTheBoard`, `makeStanding_setsFlagClearsTriageAndHorizon`, `place_setsHorizon_butStandingTasksIgnoreIt`, `isOverdue_exemptForStandingTasks`, `standingTask_hiddenUntilItsFilterIsOn`, `revealsAreIndependent`, `untriagedStaysHidden_whenOnlyOtherFlagsAreRevealed`, `untriagedTask_revealedByItsOwnFilter`, `needsTriageReveal_doesNotRevealTheOtherHiddenKinds` |

| Deleted-model safety in detail views: `ModelLiveness.isDead(isDeleted:hasContext:)` and the `PersistentModel.isDeletedOrDetached` wrapper report a model deleted in **either** window — `isDeleted` is true only pre-save, `modelContext` is nil only post-save, and `ModelContext.liveModel` covers pre-save only; `persistentModelID` stays readable throughout. `MinutesDetailView`, `TaskDetailView`, `ProjectDetailView`, `PersonDetailView`, `ContentNoteDetailView` and `CurationView` short-circuit `body` on it and render `DeletedEntityPlaceholder`, so deleting an open entity can no longer SIGTRAP | Workspace / deleted-model safety | `gbpDiaryTests/Domain/ModelLivenessTests.swift` | `isDead_whenFlaggedDeletedOrDetached`, `liveModelIsTrue_forAnInsertedModel`, `deletedBeforeSave_isDetectedByIsDeleted`, `deletedAfterSave_isDetectedByDetachment`, `persistentModelID_staysReadableAfterDeletion` |

| Board hierarchy: `BoardHierarchy.placementTargets` returns the task's **whole open subtree** (closed tasks skipped at every level; a task with no open descendants places just itself; cycle-safe), and `BoardHierarchy.rows` lays a lane's members out as nested rows — pulling in each member's **ancestor chain as context** (`isContext`, no horizon) but never its siblings, ordering top-level groups by `planSortOrder` (a context root takes its earliest member's) and rows within a group by `Task.sortOrder` | Task management / planning board | `gbpDiaryTests/Domain/BoardHierarchyTests.swift` | `placingATask_placesItsWholeOpenSubtree`, `placement_skipsClosedTasksAtEveryLevel`, `placement_withNoOpenDescendants_placesJustTheTask`, `placement_isCycleSafe`, `wholeSubtreeInALane_nestsUnderItsParent`, `loneSubtask_showsAncestorsAsContext_notSiblings`, `deepSubtask_pullsInTheWholeAncestorChain`, `groupsOrderByPlanOrder_childrenByTreeOrder`, `contextRoot_takesTheEarliestOrderOfItsMembers`, `noMembers_isEmpty` |

| Re-parenting guard: `TaskParenting.subtree(of:childrenByParent:)` returns a task's own subtree (itself + all descendants, cycle-safe) and `wouldCreateCycle(taskID:newParentID:childrenByParent:)` is true when the proposed parent is the task itself or **any** descendant (not just a direct child); parenting to an ancestor or an unrelated task is allowed. `TaskEditorSheet`'s Parent picker offers only non-cycling open tasks | Task management / task tree | `gbpDiaryTests/Domain/TaskParentingTests.swift` | `subtree_includesSelfAndAllDescendants`, `subtree_ofALeaf_isJustItself`, `cannotParentATaskToItself`, `cannotParentATaskToItsOwnChild`, `cannotParentATaskToADeepDescendant`, `parentingToAnUnrelatedTaskIsFine`, `parentingToAnAncestorIsAllowed`, `existingCycleInTheDataTerminates` |

| Derived Due & overdue group: `BoardDueGroup.includes` admits a task only when it is open, **triaged**, **not waiting**, **not standing** and **unplaced**, with `dueAt` today or earlier; `members` sorts most-overdue-first, stable on ties. Nothing is written — the group is recomputed every render, so `planHorizon` stays manual. The same helper drives the Triage list's due section | Task management / planning board | `gbpDiaryTests/Domain/BoardDueGroupTests.swift` | `dueTodayAndOverdue_areIncluded_futureIsNot`, `closedTasksAreExcluded`, `untriagedTasksAreExcluded`, `waitingTasksAreExcluded`, `standingTasksAreExcluded`, `alreadyPlacedTasksAreExcluded`, `membersAreSortedMostOverdueFirst`, `membersIsStableForEqualDueDates`, `emptyInput_isEmpty` |
| Triage is a gate: `BoardPlacement.canPlace(isOpen:needsTriage:)` allows placement only for open, triaged tasks, and `Task.place` no longer marks anything reviewed (reversing the earlier "planning doubles as triage" rule). Standing tasks carry **no due date** — `makeStanding()` clears `dueAt`, since `isOverdue` is exempt for standing tasks but `isDueToday` is not, so a lingering date would surface as "due today" for ever | Task management / planning board | `gbpDiaryTests/Domain/BoardPlacementTests.swift`, `gbpDiaryTests/Models/TaskComputedPropertyTests.swift` | `canPlace_requiresAnOpenTriagedTask`, `placingDoesNotReviewATask`, `makeStanding_setsFlagClearsTriageAndDueDate_keepingAnyHorizon` |

| Board drag payload + membership: `.draggable` carries one item, so a multi-card board drag encodes the whole selection into a single `BoardDragPayload` string — `encode`/`decode` round-trip one or many ids in order, flatten several payloads, drop repeats and ignore unparseable pieces (a stray text drop must not half-apply). `Task.isOnBoard` (`planHorizon != nil`) is the one rule shared by the board, the table's italic+muted styling and the **Not on board** filter | Task management / planning board | `gbpDiaryTests/Domain/BoardDragPayloadTests.swift` | `roundTripsASingleID`, `roundTripsAManyIDSelectionInOrder`, `flattensSeveralPayloads`, `dropsRepeatsAndKeepsFirstPosition`, `ignoresUnparseablePieces`, `isOnBoard_followsPlanHorizon` |
