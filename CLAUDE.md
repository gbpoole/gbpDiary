...
section (`DayPageContent.emailsSection` in `DayView.swift`) with a refresh button (`DaySectionHeader`'s
`systemImage:onAction:`) that reads that day's **Inbox + Sent** messages from **Mail.app** and lists
them (`DayEmailRow`). We piggyback on Mail (which already holds the OAuth-authenticated Gmail/M365
...
shown as a yellow "Unrecognized" chip. `DayEmailRow` shows a person chip (green resolved / yellow
unrecognized) + project chips; tapping a row opens **`EmailReviewSheet`** (`Views/DayTab/`), the
management workspace: a macOS `List(selection: Set<PersistentIdentifier>)` with a **Show dismissed**
