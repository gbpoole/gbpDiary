...
triage a done task); *Triage* is a **multi-selectable** `List` of inbox tasks (oldest first) with its own `BulkActionBar` — **Mark reviewed / Add to board / Cancel / Delete** — where mark-reviewed honours the same rule as the row button (a task needs a project first) and **reports how many it skipped**; rows are **`TaskTriageRow`**
