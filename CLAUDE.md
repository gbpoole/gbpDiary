...
| Each `NoteBlock.kind == .image` block is its own single-image group; consecutive images are NOT merged. Text blocks remain standalone. All groups retain original block indices. | Note block rendering/export | `gbpDiaryTests/Models/ValueTypesTests.swift` | `noteBlock_computeGroups_eachImageIsOwnGroup` |
