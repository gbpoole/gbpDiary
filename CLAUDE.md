...
`WorkspaceTab` is `.diary | .chat | .triage | .tasks | .projects | .people | .institutions | .meetings | .documents | .content | .images | .tags | .timesheet` plus entity cases `.project/.person/.institution/.minutes/.document/.contentNote(PersistentIdentifier)`. Category tabs render the existing list/tool views (`.chat` → `ChatView`, `.triage` → `EmailTriageView`, `.content` → `ContentListView`); the **Triage** sidebar item carries a `.badge` of the store-wide un-triaged (`unclassified`) email count; entity tabs resolve the model via `modelContext.model(for:)` and render its `…DetailView` (in non-sheet mode; `.contentNote` → `ContentNoteDetailView`). `reveal(note:)` sends a content note to its own `.contentNote` tab (checked before the project fallback).
...
prompt takes the top ~10. When the question asks about time **and** names an interval, `ChatView` builds
`[ChatTimeRecord]` from task time entries, standalone email time entries, and meetings, and
`ChatTimeTotalsComputer` sums them **deterministically over that interval** (deduped by source key,
per-project + overall) into an **authoritative prompt block** the model must report verbatim — the model
never sums. Whole-project/all-time totals are out of scope. `ChatPromptBuilder.build` (given
...
| Time-report lens: `ChatTimeTotals.compute` accepts an optional interval (nil = all time) and reports per-project hours + `distinctWeeks` (distinct calendar weeks with logged time); `report(...)` renders a deterministic answer (single-project sentence or per-project list + total); the pipeline answers time questions from this with no model and never degrades / never short-circuits on empty retrieval | Chat / time report | `gbpDiaryTests/Domain/ChatTimeTotalsTests.swift`, `gbpDiaryTests/Domain/ChatEvalTests.swift` | `distinctWeeks_countsDistinctCalendarWeeks`, `compute_nilInterval_isAllTime`, `report_singleProjectAndPerProject`, `totals_computedDeterministicallyOverInterval`, `timeReport_allTime_perProject_neverDegrades`, `followUp_inheritsPriorProjectIntervalAndKind` |
