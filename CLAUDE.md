...
- `TaskDetailView(task:asSheet:)` — the `.task` tab (opened by double-clicking a `TasksView` row, like Project/Person), page-styled like `ProjectDetailView` (`AppTheme.background`, pinned header + `DayActionBar`, scrolling `DaySectionHeader` sections). Header: summary title, a metadata chip row (status via `TaskStatusMenu`, project/assignee/priority/due/scheduled/logged chips), and optional notes. `DayActionBar` actions: **Edit** (→ `TaskEditorSheet`) and **Add time entry** (→ `LogTimeSheet(presetTask:)`). The primary section is **Time Log** — a single newest-first list merging the task's **`TaskTimeEntry`s and the `FocusBlock`s it backs** (a block row shows its **net** hours via `TimeLedgerProjection.blockNet`), each row clickable (`workspace.revealTimeEntry` / `revealFocusBlock`) to jump to the diary where it lives — plus a **Subtasks** section when present. The header "Logged" chip and the total use `TaskTimeReport.totalHours` (entries + block net). Metadata editing stays in `TaskEditorSheet` (a task has far more fields than Project/Person), so this page keeps an Edit action rather than full inline editing.
