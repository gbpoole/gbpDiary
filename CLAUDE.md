...
`WorkspaceTab` is `.diary | .chat | .triage | .tasks | .board | .projects | .people | .institutions | .meetings | .documents | .content | .images | .tags | .timesheet` plus entity cases `.project/.person/.institution/.minutes/.document/.contentNote/.task/.curation(PersistentIdentifier)` (`.curation` carries the **root project** of a curation walk and sits under the Projects category). Category tabs render the existing list/tool views (`.chat` → `ChatView`, `.triage` → `EmailTriageView`, `.content` → `ContentListView`); the **Emails** sidebar item (`.triage`) carries a `.badge` of the store-wide un-triaged (`unclassified`) email count; entity tabs resolve the model via `modelContext.model(for:)` and render its `…DetailView` (in non-sheet mode; `.contentNote` → `ContentNoteDetailView`, `.task` → `TaskDetailView`). `reveal(note:)` sends a content note to its own `.contentNote` tab (checked before the project fallback). **`revealTimeEntry(_:)`** / **`revealFocusBlock(_:)`** focus the diary on a logged `TaskTimeEntry`'s (resp. a `FocusBlock`'s) day (Day mode) and set `DiaryState.scrollTargetEntryId` / `scrollTargetBlockId` so the Activity section scrolls to it (each `ActivityEntryRow` carries `.id(entry.id)`, each `FocusBlockRow` `.id(block.id)`) — used by the task detail page's Time Log.
...
- `BoardView` — the **Board** sidebar page (`.board`): a global, cross-project **planning board** of four macOS columns — **Inbox / Today / This Week / Maybe** — partitioned by the pure `BoardPartition` (standing + closed tasks excluded; a task still needing triage lands in Inbox whatever its horizon; a reviewed task with **no** horizon is off-board backlog and shows in no column). **Placement is manual and never expires** — nothing auto-demotes at a day or week boundary. Drag a card to a column (or use its context menu) to set `planHorizon`; `BoardPlacement` holds the two rules: a placed task **appends to the end** of the target column (`appendOrder`), and placing a task that was in the Inbox **also reviews it** (`shouldReview`) — choosing when to do something is the triage decision. The Inbox column is a drag source only, never a drop target. Double-click a card opens the task tab. The diary's `DayTaskPanel` remains the execution view; this is the planning surface.
...
| Planning board: `BoardView` (`.board` category tab, token `"board"`) renders `BoardPartition`'s Inbox/Today/This-Week/Maybe columns; dropping a card calls `BoardPlacement.appendOrder` so it lands at the **end** of the target column (max+1, not count) and `BoardPlacement.shouldReview` so a task placed **out of the Inbox is also marked reviewed** and leaves that column; standing tasks can never be placed (`Task.place` is a no-op for them) | Task management / planning board | `gbpDiaryTests/Domain/BoardPlacementTests.swift`, `gbpDiaryTests/Domain/WorkspaceSessionTests.swift` | `appendOrder_goesAfterEverythingInTheColumn`, `shouldReview_onlyWhenComingFromTheInbox`, `placingAnInboxTask_reviewsItAndLeavesTheInbox`, `standingTaskStaysOffTheBoard`, `categoryTokens_roundTrip` |
