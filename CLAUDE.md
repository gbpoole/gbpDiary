...
`WorkspaceTab` is `.diary | .chat | .triage | .tasks | .projects | .people | .institutions | .meetings | .documents | .content | .images | .tags | .timesheet` plus entity cases `.project/.person/.institution/.minutes/.document/.contentNote/.task(PersistentIdentifier)`. Category tabs render the existing list/tool views (`.chat` → `ChatView`, `.triage` → `EmailTriageView`, `.content` → `ContentListView`); the **Emails** sidebar item (`.triage`) carries a `.badge` of the store-wide un-triaged (`unclassified`) email count; entity tabs resolve the model via `modelContext.model(for:)` and render its `…DetailView` (in non-sheet mode; `.contentNote` → `ContentNoteDetailView`, `.task` → `TaskDetailView`). `reveal(note:)` sends a content note to its own `.contentNote` tab (checked before the project fallback). **`revealTimeEntry(_:)`** / **`revealFocusBlock(_:)`** focus the diary on a logged `TaskTimeEntry`'s (resp. a `FocusBlock`'s) day (Day mode) and set `DiaryState.scrollTargetEntryId` / `scrollTargetBlockId` so the Activity section scrolls to it (each `ActivityEntryRow` carries `.id(entry.id)`, each `FocusBlockRow` `.id(block.id)`) — used by the task detail page's Time Log.
